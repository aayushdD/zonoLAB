...
% Extract unique edges from convex hull facets
    ...
    for i = 1:size(K, 1)
        ...
        for j = 1:n_vertices
            ...
            if v1 > v2
                [v1, v2] = deal(v2, v1);
                %fancy way of exchanging the vertices if the condition is
                %satisfied.
            end
            ...
            if ~isKey(edge_set, key)
                ...
                %finding how far away these two vertices are. 
                if norm(vec) > 1e-10 % magnitutde of the distance and remove very close ones.
...
% Process edges to find candidate generators (Algorithm 3 lines 1-14)
    ...
    for i = 1:length(edges)
        %The function below finds the unit vector in the direction of the
        %edge. we want to do that because we will later check if two edges
        %are parallel. For example [0,2] and [0,-2] both are y axis which
        %will both be [0,1] after this and we will know they are parallel. 
        [dir, vec] = normalizeDirection(edges{i}.vector);
        directions(i,:) = dir;
    end
    
    % directions will be an array that will give directions of each edges.
    % This is done by using the normalizeDirection function above and using
    % index i to populate the direction of the edge in the direction
    % array. If two edges are parallel than they will have the same entry
    % in the direction array which will be captured by the unique function
    % below.
    [unique_dirs, ~, ic] = unique(round(directions*1e8)/1e8, 'rows', 'stable');

    %unique dirs will contain only the unique directional edges. For
    %example a square set will have 2x2 unique dirs array.
